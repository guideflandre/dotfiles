...
def get_default_remote():
    ...
    config = load_config()
    host = config.get('DEFAULT', 'remote_host', fallback='')
    user = config.get('DEFAULT', 'remote_user', fallback='')
...
def main(ctx, file, directory, mode, recursive):
    """
    ...
    if ctx.invoked_subcommand is None:
        ...
        # Handle transfer
        transfer_file(file, directory, mode, recursive)


def transfer_file(file, directory, mode, recursive):
    ...
    # Load config for default remote
...
def set_host(host, user):
    ...
    """
    cfg = load_config()
    cfg['DEFAULT']['remote_host'] = host
    cfg['DEFAULT']['remote_user'] = user
    save_config(cfg)
